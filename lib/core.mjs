...
function buildSystem(skillsText, sel, fromVer, target) {
  return [
...
async function planLlm(cfg, skillsText, sections, files) {
  const summary = [...sections.entries()]
    .map(([h, body]) => `## ${h}\n${body.slice(0, 40).join('\n')}`)
    .join('\n\n');
  const system = `你是 Minecraft 模組版本遷移規劃師。${skillsText}\n\n任務：審視以下專案檔案清單與環境文檔摘要，判斷哪些檔案需要修改。只輸出 JSON：{"files": ["相對路徑", ...], "risks": "風險說明"}。`;
  const user = `專案檔案清單：\n${files.join('\n')}\n\n環境文檔摘要：\n${summary}`;
...
async function editLlm(cfg, skillsText, sections, rel, content, errorLog, fromVer, target, stats) {
  const sel = relevantSections(sections, rel, cfg.token_budget_chars);
  const system = buildSystem(skillsText, sel, fromVer, target);
...
// ---------- 主流程（AGENT-1） ----------
// p: { project, fromVer, target, env, provider, model, apiKey, maxIterations, dryRun, buildCmd, noBuild, force, config, skillsDir }
export async function runMigration(p, onEmit) {
  ...
  const envPath = p.env
    ? path.resolve(p.env)
    : path.join(RES_DIR, 'mcenv', `${fromVer}_to_${target}.md`);
  ...
  const project = path.resolve(p.project);
  if (!fs.existsSync(project) || !fs.statSync(project).isDirectory()) fail(`專案目錄不存在：${project}`);
  const mock = cfg.provider === 'mock';

  // 1. 載入環境
  emit('log', `載入環境文檔：${path.basename(envPath)}｜Skills ${skillsText ? skillsText.split('### Skill').length - 1 : 0} 個｜供應商：${cfg.provider}`);
  // AGENT-6：git 安全
  ...
  emit('log', `掃描專案：${files.length} 個候選檔案`);
  ...
  if (mock) {
    ...
    const pl = await planLlm(cfg, skillsText, sections, files);
    planned = pl.files;
    risks = pl.risks;
  ...
  for (const rel of toDo) {
    ...
    if (mock) {
      ...
      neu = await editLlm(cfg, skillsText, sections, rel, content, null, fromVer, target, stats);
    }
  ...
  while (buildStatus === 'fail' && iterations < cfg.max_iterations) {
    ...
    for (const rel of Object.keys(changes)) {
      ...
      else neu = await editLlm(cfg, skillsText, sections, rel, content, buildLog, fromVer, target, stats);
  ...
  writeReport(project, { fromVer, target, cfg, envPath, files, planned, skipped, changes, iterations, buildFailed, buildStatus, buildLog, review, stats });
