...
async function editLlm(cfg, skillsText, sections, rel, content, errorLog, fromVer, target, stats) {
  ...
  emit('warn', `${rel} 過大（${content.length} 字元），分段遷移中…`);
...
// ---------- 主流程（AGENT-1） ----------
// p: { project?, files?, fromVer, target, env, provider, model, apiKey, maxIterations, dryRun, buildCmd, noBuild, force, config, skillsDir }
...
export async function runMigration(p, onEmit) {
  ...
  const project = path.resolve(explicitFiles ? commonRoot(p.files) : p.project || '');
