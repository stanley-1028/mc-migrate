...
import { generatePairDoc } from '../gen-env.mjs';

const TOOL_DIR = path.dirname(fileURLToPath(import.meta.url));
// 資源預設位置：lib/core.mjs 的上層（CLI 為專案根，桌面版為 app/ 根）
const RES_DIR = path.resolve(TOOL_DIR, '..');
// 官方文檔自動生成/快取目錄（打包版 asar 唯讀，一律寫使用者目錄）
const USER_ENV_DIR = path.join(os.homedir(), '.mc-migrate', 'mcenv');
...
async function migrateJar(p, onEmit) {
  ...
  // 逐一列舉頂層條目（避免 tar -C dir . 的 ./ 前綴導致載入器找不到 fabric.mod.json），並排除工具產物
  const entries = fs.readdirSync(extractDir).filter((n) => n !== STATE_DIR);
  const pk = spawnSync('tar', ['--format', 'zip', '-cf', outJar, '-C', extractDir, ...entries], { encoding: 'utf8' });
...
// 由所選資料夾向上找真正的專案根（有 gradle 檔的層級）
function findProjectRoot(startDir) {
  let dir = path.resolve(startDir);
  for (let i = 0; i < 10; i++) {
    const names = fs.existsSync(dir) ? fs.readdirSync(dir) : [];
    if (names.some((n) => /^(settings|build)\.gradle(\.kts)?$/.test(n) || n === 'gradlew' || n === 'gradlew.bat')) return dir;
...
// 偵測來源載入器：整棵專案掃描 import/建構檔 + 檔名線索（fabric.mod.json / mods.toml）
function findLoader(project) {
  const root = findProjectRoot(project);
  const scan = scanProject(root);
  const text = scan.map((f) => readHead(path.join(root, f))).join('\n');
  const d = detectLoader([text]);
  if (d) return d;
  for (const f of scan) {
    const base = f.split('/').pop();
    if (base === 'fabric.mod.json') return 'fabric';
    if (base === 'neoforge.mods.toml') return 'neoforge';
    if (base === 'mods.toml') return 'forge';
  }
  return '';
}
...
export async function runMigration(p, onEmit) {
  ...
  // 偵測＝向上定位專案根後整棵掃描（import、build.gradle 插件、fabric.mod.json、mods.toml）
  const fromLoader = p.loaderFrom ? p.loaderFrom : loaderTo ? findLoader(project) : '';
  if (loaderTo && !fromLoader) {
    fail('無法偵測來源載入器：專案內找不到載入器標記（import、build.gradle 插件、fabric.mod.json、mods.toml 等）。請在「來源載入器」手動指定（Forge / Fabric / NeoForge），或用 --env 指定環境文檔');
  }
  let envPath = p.env ? path.resolve(p.env) : null;
  const docName = loaderTo ? `${fromLoader}_${fromVer}_to_${loaderTo}_${target}.md` : `${fromVer}_to_${target}.md`;
  if (!envPath) {
    for (const dir of [path.join(RES_DIR, 'mcenv'), USER_ENV_DIR]) {
      const cand = path.join(dir, docName);
    ...
    if (!envPath && !loaderTo) {
      emit('log', `環境文檔不存在，以 Mojang 官方資料自動生成 ${docName}…`);
      const r = await generatePairDoc(fromVer, target, USER_ENV_DIR);
      envPath = r.docPath;
      emit('log', `已生成官方文檔：${envPath}`);
    }
  }
  if (!envPath) {
    const dir = path.join(RES_DIR, 'mcenv');
    const existing = fs.existsSync(dir) ? fs.readdirSync(dir).join('、') : '無';
    fail(`找不到環境文檔 ${docName}（mcenv/ 現有：${existing}）。跨載入器文檔需人工撰寫，格式見 TEMPLATE.md；版本升級文檔會自動以 Mojang 官方資料生成。`);
  }
