...
export async function runMigration(p, onEmit) {
  ...
  const fromLoader = loaderTo
    ? detectLoader(explicitFiles ? [...fileContents.values()] : files.map((f) => readHead(path.join(project, f))))
    : '';
  if (loaderTo && !fromLoader) {
    fail('無法偵測來源載入器：請確認檔案含對應 import（net.minecraftforge / net.fabricmc / net.neoforged），或用 --env 直接指定環境文檔');
  }
  const envPath = p.env
    ? path.resolve(p.env)
