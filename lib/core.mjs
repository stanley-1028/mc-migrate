...
async function editLlm(cfg, skillsText, sections, rel, content, errorLog, fromVer, target, stats, abort) {
...
const BINARY_EXTS = new Set([
  '.jar', '.class', '.zip', '.7z', '.rar', '.png', '.jpg', '.jpeg', '.gif', '.webp',
...
export async function runMigration(p, onEmit) {
  ...
  for (const rel of toDo) {
    ...
    if (mock) {
      ...
      neu = await editLlm(cfg, skillsText, sections, rel, content, null, fromVer, target, stats, p.abort);
    }
  ...
  while (buildStatus === 'fail' && iterations < cfg.max_iterations) {
    ...
    for (const rel of Object.keys(changes)) {
      ...
      else neu = await editLlm(cfg, skillsText, sections, rel, content, buildLog, fromVer, target, stats, p.abort);
